...
// [[Rcpp::export]]
vector<vector<double> > disma2d_truncated(
	vector<double> &dist_train,
...
// [[Rcpp::export]]
vector<vector<double> > disma3d_truncated(
	vector<double> &w_train,
	vector<double> &dist_train,
	vector<double> &sim_train,
	vector<double> &w_test,
	vector<double> &dist_test,
	vector<double> &sim_test,
	int &N_train,
	int &N_test) {
	...
	for (int i=0; i < N_test; i++){
		for (int j=0; j < N_train; j++){
			distance_train_test[i][j] = distance3d(
				w_train[j],
				dist_train[j],
				sim_train[j],
				w_test[i],
...
// [[Rcpp::export]]
vector<vector<double> > disma2d_centroid(
	vector<double> &dist_train,
	vector<double> &sim_train,
	vector<double> &dist_test,
	vector<double> &sim_test,
...
vector<vector<double> > K_centroid_zeros(vector<double> &dist, vector<double> &sim, vector<int> &id, int N, int K){
    vector<vector<double> > K_coords(K, vector<double>(2,0));
    int count_k;
    for (int k=0; k < K; k++){
        count_k=0;
        for (int i=0; i < N; i++){
            if (id[i] == k+1){
                // K_coords[k][0] += pow(dist[i], 2) * dist[i] / dist[i];
                // K_coords[k][1] += pow(sim[i], 2) * sim[i] / sim[i];
                K_coords[k][0] += dist[i];
                K_coords[k][1] += sim[i];
                count_k++;
            }
        }
        // K_coords[k][0] = sqrt(K_coords[k][0]);
        // K_coords[k][1] = sqrt(K_coords[k][1]);
        K_coords[k][0] /= count_k;
        K_coords[k][1] /= count_k;
    }
    return K_coords;
...
Rcpp::List distma2zero(vector<double> &dist_train,
    ...
    vector<vector<double> > K_coords = K_centroid_zeros(dist_train, sim_train, id_train, N_train, K);
    vector<vector<double> > leave2centroid(2*K, vector<double>(max(N_test, N_train),0));
