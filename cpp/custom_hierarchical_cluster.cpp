    ...
    for (int i=0; i < N-1; i++){
        for (int j=0; j < N-i-1; j++){
            if (matrix[i][j] > 0){
                ...
                average_dis.push_back(matrix[i][j]);
...
long double average_linkage(vector<int_fast32_t> &m1, vector<int_fast32_t> &m2, vector<vector<long double> >  &dist){
    ...
    for (int i=0; i < m1.size(); i++){
        for (int j=0; j < m2.size(); j++){
            ...
            if (m1[i] == m2[j]){
                ...
                tmp = dist[m2[j]][m1[i]-m2[j]-1];
                if (tmp > 0){
                    dist1 += tmp;
                    count++;
                }
            } else{
                tmp = dist[m1[i]][m2[j]-m1[i]-1];
                if (tmp > 0){
                    dist1 += tmp;
                    count++;
                }
    ...
    if (count > 0){
        dist1 /= count;
        // cout << "Warning: link communities are totally unconneted\n ";
        // cout << dist1 << endl;
    }
...
//// Paralelization project: still needs work
template<typename I>
...
int main(){
    ...
    //strings
    string out_path;
    const string csv_path = "../CSV/merged/similarity/tracto2016/zz_model/sim_l10.csv"; // merged/similarity/tracto2016/zz_model/
    //ints
	const int_fast32_t leaves = 6868; // 6868
    const int_fast32_t threshold = 0; //

    //// Start code
    cout << "Read similarity matrix\n";
    vector<vector<long double> > distance = read_csv(csv_path);
    cout << "Finished";
    printf("leaves: %i   threshold: %i\n", leaves, threshold);

    cout << "Customized similarity\n";
    custom_dis_matrix(distance, leaves);
    cout << "Finished\n";

    cout << "START CUSTOMIZED HIERARCHICAL ALGORITHM\n";
    ...
    cout << "Saving the height and merge vectors\n";
    out_path = "../CSV/merged/hclust/tracto2016/zz_model/TH_0_INDEPENDENT/merge_.csv";
    out_csv_2d(merge, out_path);
    out_path = "../CSV/merged/hclust/tracto2016/zz_model/TH_0_INDEPENDENT/height.csv";
