...
get.path <- function(path, folder){
  ...
  file.path <-
    "{folder}/{subject}/{system}/{version}/{dist.ver}/{model}/{variant}"
...
write.csv(
  ...
  file.path(
    '..',
    'CSV',
    'imputation_{model}_{variant}_{dist.ver}.csv' %>%
      sformat(format.path)
    ), row.names = F
  )
...
for (e in 1:length(fln.col)) {
  if(grepl('X',fln.col[e], fixed = T)){
    col_e <- stringr::str_split(fln.col[e], 'X') %>%
      unlist()
    fln.col[e] <- col_e[2]
  }
}
write.csv(
  ...
  file.path(
    '..',
    'CSV',
    'imputation_{model}_{variant}_{dist.ver}_clabel.csv' %>%
      sformat(format.path)
    ),
    row.names = F
  )
