get.tracto2016 <- function(fln.labels) {
  tracto2016 <- read.csv(
    "../CSV/Distances/dis_tra_11_mn_ld_2022labels.csv"
  ) %>%
    as.matrix()
  tracto.labels <- tracto2016[, 1] %>%
    tolower()
  tracto2016 <- tracto2016[, 2:108] %>%
    as.numeric() %>%
    pracma::Reshape(107, 107)
  tracto2016 <- tracto2016 %>%
    as.data.frame()
  colnames(tracto2016) <- tracto.labels
  rownames(tracto2016) <- tracto.labels
  map3d <- read.csv(
    "../CSV/Distances/DistanceMatrix_Map3D_10feb2022_107x107.csv"
  ) %>%
    as.matrix()
  map3d.labels <- map3d[, 1] %>%
    tolower()
  map3d <- map3d[, 2:108] %>%
    as.numeric() %>%
    pracma::Reshape(107, 107)
  ...
  map3d <- map3d[tracto.labels, tracto.labels] %>%
    as.matrix()
  ...
  lm.df <- data.frame(
    x = map3d$weight[!is.na(tracto2016$weight)],
    y = tracto2016$weight[!is.na(tracto2016$weight)]
  )
  ...
  colnames(tracto2016) <- tracto.labels
  rownames(tracto2016) <- tracto.labels
  tracto2016 <- tracto2016[fln.labels, fln.labels] %>%
    as.matrix()
  return(tracto2016)
}