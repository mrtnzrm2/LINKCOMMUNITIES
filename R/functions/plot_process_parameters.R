plot.process.parameters <- function(path.plot, foldername, hclust.features){
  ...
  hclust.features <- hclust.features[!is.na(hclust.features$SS),]
  Curves <- colnames(hclust.features) %>% .[2:length(.)]
  for (curve in Curves){
    best.height <- find.height(hclust.features, curve=curve, max.wire=1,
                               l_continuous = F, min.wire = 0, jump = 0.2)
    if (pracma::strcmp(curve, 'SS')){
  ...
  fig <- plotly::plot_ly(data = hclust.features, x = ~K) %>%
    plotly::add_trace(y=~SS, name="S/S ~ K", mode= "lines+markers",
                      marker = list(size = 3,
                                    color = "rgba(255, 182, 193, .9)",
                                    line = list(color = 'rgba(152, 0, 0, .8)',
                                                width = 2)))
  fig <- plotly::layout(fig,  xaxis = list(type = "log"))
  htmlwidgets::saveWidget(plotly::as_widget(fig), sprintf("%s/%s/%s/%s.html", path.plot, foldername, 'VariableEvolution', 'SS_logK'))
}