...
index, name, sequence, view, x, y, name2 = np.loadtxt(
  "utils/flatmap/flatmapdataframe2c.csv",
  skiprows=1, delimiter=',', unpack=True, dtype=str
)
...
for index in range(totalAreas):
    ...
    pattern = ''
    axFlatmap.add_patch(plt.Polygon(vertices, closed=True, edgecolor=[0, 0, 0], linewidth=.5, facecolor=color, hatch=pattern))
...
plt.savefig(
  "../plots/{}/{}/{}/AVERAGE_full_l10/flatmap/{}_link_communities_k_{}_{}_b.pdf".format(foldername, distance, model, filename, k, ltype)
)
plt.close()
